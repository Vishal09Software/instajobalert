                        ...
                        <form action="{{ route('skills.update', $skill->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <x-input type="text" name="title" id="title" :value="$skill->title" required autofocus placeholder="Enter skill title" />
                                    @error('title')
                                ...
                                <div class="col-md-4 mb-3">
                                    ...
                                    <x-input type="text" name="slug" id="slug" :value="$skill->slug" required placeholder="Enter skill slug" />
                                    @error('slug')
                                        <small class="text-danger d-block">{{ $message }}</small>
                                    @enderror
                                </div>
                                <div class="col-md-4 mb-3">
                                    ...
                                    <x-input type="text" name="icon" id="icon" :value="$skill->icon" placeholder="Enter icon class e.g. bi bi-folder" />
                                    @error('icon')
                                        <small class="text-danger d-block">{{ $message }}</small>
                                    @enderror
                                </div>
                                ...
                                <div class="form-check form-switch form-switch-md mb-3">
                                    <input class="form-check-input code-switcher" type="checkbox" id="9"
                                        name="status" value="1" {{ $skill->status ? 'checked' : '' }}>
